...
######################################################
# 
# FUNCTIONS TO MANAGE BUNDLED TASKS
# 
######################################################
######################################################
...
def importItemsBundled(tbl, data):
	...
	for i in data:
		...
		for i in range(0,3):
			N = random.randint(1, 25)
			task = { str("Task_" + str(i)): { "Status": 'todo', "Script": str('seq ' + str(N)) } }
			taskScript.update(task)
		taskScript = json.dumps(taskScript)


		# Import item
		item = { "itemID": str(itemID), "taskID": str(taskID), "ID_Index": str(counter), "TaskScript": taskScript, "lockID": 'todo', "Log": "NULL", "Lock_Date": "NULL", "Done_Date": "NULL", "Log_Length": "NULL" }
		counter+=1
		itemList_import.append(item)
		response = table.put_item(Item = item, ReturnConsumedCapacity = 'TOTAL')
		importResponse.append(response)
...
def updateBundledItem(task, tbl, itemID, lockID, itemImport):
	...
	if task == 'done':
		...
		response = table.update_item(
			...
			ExpressionAttributeValues = {
				":lockingID": str("Done_" + str(lockID)),
				":dateDone": str(now)
			},
	...
	# Handle individual task
			...
			# Execute update
			taskData = itemImport[0]
			logLength = itemImport[1]
			taskScript = itemImport[2]
			response = table.update_item(
				Key = {'itemID': itemID},
				ExpressionAttributeNames = {
				"#LogLen": "Log_Length",
				...
				ExpressionAttributeValues = {
					":logs": json.dumps(taskData),
					":logLen": json.dumps(logLength),
					":TaskScript": json.dumps(taskScript),
				},
				UpdateExpression = "SET #LogLen = :logLen, #Logging = :logs, #TaskScript = :TaskScript",
				ReturnValues="UPDATED_NEW"
			)
